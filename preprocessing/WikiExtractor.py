...
'''
	Code source of wiki extractor. Modified to preserve categories information of the article.
'''
...
def make_anchor_tag(match):
    ...
    link = match.group(1)
    colon = link.find(':')
    if colon > 0 and link[:colon] not in acceptedNamespaces and not(re.search("^Category:.*$",link)):
        return ''
    if colon == 0:
        # drop also :File:
